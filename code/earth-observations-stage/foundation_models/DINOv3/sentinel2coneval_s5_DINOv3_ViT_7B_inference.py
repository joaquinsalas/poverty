# dinov3_vit7b_s5_infer_emissions.py
# --- top of file (keep your env vars) ---
import os
os.environ["CUDA_VISIBLE_DEVICES"] = "1"
os.environ["CODECARBON_SAVE_TO_API"] = "false"
os.environ["CODECARBON_SAVE_TO_FILE"] = "false"
os.environ.setdefault("PYTORCH_CUDA_ALLOC_CONF", "expandable_segments:True")  # helps fragmentation
...
# Splits (only to build the test set)
TRAIN_SPLIT = 0.50
VAL_SPLIT   = 0.20

# Sentinel-2A config
D_INPUT = 12
S2_SCALE = 5e-5  # or 1e-4 depending on how TIFFs were written

# DINOv3 config (7B is heavy; ensure GPU/VRAM is enough)
DINO_PRETRAINED = "facebook/dinov3-vit7b16-pretrain-sat493m"
FREEZE_DINO = True
IMG_SIZE = 224  # must be multiple of 16 (ViT/16)

# Choose RGB-like bands or set None to use PCA(12->3)
RGB_BANDS = (3, 2, 1)
...
class Sentinel2ADataset(Dataset):
    ...
    CSV needs:
      ...
      - 'target' or 30 columns prediction_01..prediction_30 (averaged)
    Each .tif is (C,H,W) with C==12. Convert to 3-ch image via band select or PCA(12->3).
...
class DinoS5Regressor(nn.Module):
    def __init__(self, dino_name=DINO_PRETRAINED, d_model=D_MODEL, n_layers=N_LAYERS,
                 ...
                 dino_device="cpu", head_device=None):
        super().__init__()
        kw = {}
        if hf_token is None:
        ...
        # 1) Load huge ViT-7B on CPU (do NOT move the whole module to CUDA)
        self.dino = AutoModel.from_pretrained(dino_name, **kw).to("cpu").eval()
        if freeze_dino:
            for p in self.dino.parameters():
                p.requires_grad = False

        self.dino_device = dino_device  # keep as "cpu"
        self.img_size = img_size

        # keep normalization buffers on the SAME device as the images we'll feed to DINO (CPU)
        ...
        d_dino = self.dino.config.hidden_size  # 4096 for ViT-7B/16
        self.prenorm = prenorm

        # 2) Small head lives on GPU if available
        head_device = head_device or ("cuda" if torch.cuda.is_available() else "cpu")
        self.enc = nn.Linear(d_dino, d_model).to(head_device)
        self.s5_layers = nn.ModuleList([S5Block(dim=d_model, state_dim=d_model, bidir=False)
                                        for _ in range(n_layers)]).to(head_device)
        self.norms = nn.ModuleList([nn.LayerNorm(d_model) for _ in range(n_layers)]).to(head_device)
        self.dropouts = nn.ModuleList([nn.Dropout(dropout) for _ in range(n_layers)]).to(head_device)
        self.head = nn.Linear(d_model, d_output).to(head_device)
    ...
    def _preprocess(self, x):
        ...
        # keep preprocessing on CPU to match DINO device
        x = x.to("cpu", non_blocking=True)
        x = torch.nn.functional.interpolate(x, size=(self.img_size, self.img_size),
                                            mode="bilinear", align_corners=False)
        x = x.clamp_(0, 1)
        x = (x - self.mean) / self.std   # mean/std are CPU buffers
        return x

    def forward(self, imgs_3chw_list):
        # 3) DINO forward on CPU
        pixel_values = self._preprocess(imgs_3chw_list)   # CPU tensor
        with torch.no_grad():
            out = self.dino(pixel_values=pixel_values, output_hidden_states=False)
        tokens = out.last_hidden_state                      # CPU
        patch_tokens = tokens[:, 1 + self.n_registers:, :]  # CPU: (B, N, d_dino)

        # 4) Move only patch tokens to GPU for the small head
        x = patch_tokens.to(self.head_device, non_blocking=True)
...
# ----------------------------
# ----------------------------
# Main (inference-only + emissions)  **CHANGES SHOWN ONLY**
# ----------------------------
def main():
    set_seed(SEED)
    cudnn.benchmark = False  # avoid large workspaces on tight VRAM
    os.makedirs(MODEL_DIR, exist_ok=True)

    # ---------- load CSV & filter to existing TIFFs ----------
    ...
    print(f"Samples kept: {stats['kept']} | missing: {stats['missing']} | "
          f"wrong_bands: {stats['wrong_bands']} | unreadable: {stats['unreadable']}")
    if len(data) == 0:
        raise RuntimeError("No valid samples after filtering. Check IMG_DIR or filenames.")

    # ---------- dataset + deterministic split (we only use test set here) ----------
    ...
    # keep tensors on CPU; DINO runs on CPU; head runs on GPU
    test_loader = DataLoader(
        ...
        pin_memory=True,
        collate_fn=collate_noresize
    )

    # ---------- model: DINO on CPU, small head on GPU if available ----------
    model = DinoS5Regressor(
        dino_name=DINO_PRETRAINED,
    ...
    # ---------- load checkpoint ON CPU (avoid GPU allocation during load) ----------
    ...
    # ---------- 2) Emissions ----------
    if OfflineEmissionsTracker is None:
        print("WARNING: CodeCarbon not installed; emissions will be NaN.")
        metrics = {
            "emissions_g_per_item_median": float("nan"),
            "emissions_g_per_item_mean":   float("nan"),
        }
    else:
        grams_samples = []
        for _ in range(REPETITIONS):
            ...
            tracker.start()
            for xb, _ in tqdm(test_loader, desc="Measuring emissions", leave=False):
                _ = model(xb)  # CPU DINO + GPU head
            emissions_kg = tracker.stop()
            grams_per_item = (np.nan if emissions_kg is None
                              else float(emissions_kg) * 1000.0 / max(1, len(test_set)))
            grams_samples.append(grams_per_item)
            if torch.cuda.is_available():
