...
# ------------------------------------------------------------------------------
# 4. EfficientNetB3 model adapted to 24 channels
# ------------------------------------------------------------------------------
def build_custom_efficientnet_b3_partial(input_shape=(224, 224, 23), l2_penalty=0.01):
    ...
    for i in range(5, len(conv_base.layers)):
        try:
            ...
            print(f"Skipped layer {i} ({layer_target.name if 'layer_target' in locals() else '?'}) — {e}")
...
model = build_custom_efficientnet_b3_partial(input_shape=(224, 224, 23))
...
# ------------------------------------------------------------------------------
# 11. Model evaluation and prediction on the test set
# ------------------------------------------------------------------------------
print("\n🔎 Loading trained model...")
model = tf.keras.models.load_model(
    "model_best_val_loss_24bands_efficientnetB3.keras"
)
# ------------------------------------------------------------------------------
...
plt.title("Comparison between true and predicted values (TEST)")
plt.grid(True)
plt.tight_layout()
plt.savefig(
    "scatter_plot_predictions_vs_true_24bands.png",
    dpi=300
)
plt.show()
...
df_pred.to_csv(
    "predictions_test_efficientnet_b3_24bands_sentinel2a.csv",
    index=False
)
