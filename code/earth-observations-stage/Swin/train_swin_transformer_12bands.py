...
# ----------------------
def build_swin_regression_model(input_shape=(96, 96, 12), embed_dim=48, window_size=6, num_heads=4, num_blocks=2):
...
def load_images(inds):
    ...
    for idx in inds:
        ...
        with rasterio.open(filename) as src:
            #img = src.read((2, 3, 4))
            img = src.read(list(range(1, 13))) 
            img = scale * img
        img = np.swapaxes(img, 0, 2)
        img = np.swapaxes(img, 0, 1)
        x_imgs.append(img)
    return np.array(x_imgs, dtype=np.float32)
...
checkpoint_best = ModelCheckpoint(
    filepath='model_best_val_loss_12bands_swin.keras',
    monitor='val_loss',
    save_best_only=True,
...
# Model and training
# --------------------------
model = build_swin_regression_model(input_shape=(96, 96, 12))  # asegúrate que esta función está definida
...
}).to_csv('training_history_swin12bands.csv', index=False)
model.save('model_final_swin12bands.keras')
...
model = tf.keras.models.load_model(
    "model_best_val_loss_12bands_swin.keras",
    custom_objects={
...
df_pred.to_csv("predictions_test_set_swin_12bands.csv", index=False)

print("Predictions exported to predictions_test_set_swin_12bands.csv")
